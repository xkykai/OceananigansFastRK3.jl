...
function time_stepper(grid, timestpr)
    model_prototype = setup_model(grid)

    if timestpr == :LeMoinRungeKutta3FPJ0
        α = β = 0
        return LeMoinRungeKutta3TimeStepper(grid, prognostic_fields(model_prototype); α, β)
    ...
    elseif timestpr == :LeMoinRungeKutta3FPJ2
        ...
        return RungeKutta3TimeStepper(grid, prognostic_fields(model_prototype))
    end
end
...
# timesteppers = [:LeMoinRungeKutta3]
...
function run_simulation(; timestpr, Δt, filename)
    ...
    timestepper = time_stepper(grid, timestpr)
...
#%%
model_l0 = setup_model(grid, time_stepper(grid, :LeMoinRungeKutta3FPJ0))
model_l1 = setup_model(grid, time_stepper(grid, :LeMoinRungeKutta3FPJ1))
model_l2 = setup_model(grid, time_stepper(grid, :LeMoinRungeKutta3FPJ2))
model_rk = setup_model(grid, time_stepper(grid, :RungeKutta3))
